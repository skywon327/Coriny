...
public class notification extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ArrayList<String> items = new ArrayList<>(Arrays.asList("현재버전", "개인정보 처리 방침", "서비스 이용약관", "개발자"));
        // CustomAdapter 클래스를 사용하여 ListView에 항목을 표시할 어댑터를 생성합니다.
        notification.CustomAdapter adapter = new notification.CustomAdapter(this, 0, items);
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            textView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if ("현재버전".equals(items.get(position))){
                        move = SubActivity.class;}
                    else if ("개인정보 처리 방침".equals(items.get(position))){
                        move = SubActivity.class;}
                    else if ("서비스 이용약관".equals(items.get(position))){
                        move = SubActivity.class;}
                    else if ("개발자".equals(items.get(position))){
                        move = SubActivity.class;}

                    // Intent를 사용하여 새로운 화면으로 이동합니다.
                    Intent intent = new Intent(notification.this, move);
                    startActivity(intent);
                }
            });
