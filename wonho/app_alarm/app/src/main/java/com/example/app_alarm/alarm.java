...
public class alarm extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ArrayList<String> items = new ArrayList<>(Arrays.asList("전체알람", "결제하기알람", "버디알람", "모임알람", "후기알람"));
        // CustomAdapter 클래스를 사용하여 ListView에 항목을 표시할 어댑터를 생성합니다.
        alarm.CustomAdapter adapter = new alarm.CustomAdapter(this, 0, items);
    ...
    private class CustomAdapter extends ArrayAdapter<String> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if ("모임알람".equals(items.get(position))){
                list_frame2.removeView(button);
                textView_sub.setText("1시간 전");
            ...
            button.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if ("전체알람".equals(items.get(position))){
                        move = SubActivity.class;}
                    else if ("결제하기알람".equals(items.get(position))){
                        move = SubActivity.class;}
                    else if ("버디알람".equals(items.get(position))){
                        move = SubActivity.class;}
                    else if ("후기알람".equals(items.get(position))){
                        move = SubActivity.class;}

                    // Intent를 사용하여 새로운 화면으로 이동합니다.
                    Intent intent = new Intent(alarm.this, move);
                    startActivity(intent);
                }
            });
